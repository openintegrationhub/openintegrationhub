...
const { formatAndRespond, errorHandler, setReqLogger, setCors } = require('./middleware');
...
class Server {
    constructor({config, logger, iam = defaultIam, eventBus}) {
        this._config = config;
        this._logger = logger;
        ...
        app.use('/components', setCors({whitelist, logger}), require('./routes/components')({iam}));
    ...
    async _subscribeToEvents() {
        const eventSubscription = new EventSubscription({eventBus: this._eventBus});
