...
describe.only('POST /components/restart/all', () => {
    let server;
    ...
    it('should start a global component by ID', async () => {
        const { body, statusCode } = await request(server.getApp()).post(`/components/global/restart/all?emitThrottle=0&restartDelay=20`);

        expect(statusCode).to.equal(200);

        expect(body).to.deep.equal({ message: '2 Components will be restarted', code: 200 });
        let updatedComponents = await Component.find({ isGlobal: true }).lean();
        console.log(updatedComponents);
        expect(updatedComponents[0].active).to.be.false;
        expect(updatedComponents[1].active).to.be.false;
        await sleep(3);
        updatedComponents = await Component.find({ isGlobal: true }).lean();
        console.log(updatedComponents);
