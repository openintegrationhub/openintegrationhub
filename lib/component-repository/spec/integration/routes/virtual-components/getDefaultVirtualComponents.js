...
const { componentVersionNotDefaultVersion } = require('./__mocks__/virtualComponentsData');
...
describe('Get Default Component versions', () => {
  ...
  it('should return two components version to the permitToken user', async () => {
    ...
    const isNotDefaultInResult = body.data.some(({_id}) => _id === notDefaultId);
    expect(isNotDefaultInResult).to.be.false;
    expect(statusCode).to.equal(200);
  });

  it('should return 4 components version to the admin user', async () => {
    const { body, statusCode } = await request(server.getApp())
      .get('/virtual-components/defaults')
