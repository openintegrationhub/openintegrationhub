...
describe('Get action from a component version', () => {
  ...
  it('should return the trigger to the permitToken user', async () => {
    const { statusCode } = await request(server.getApp())
      .get(
        `/virtual-components/${virtualComponent2._id}/${componentVersion2._id}/triggers/name3`
      )
  ...
  it('should return could not find the trigger to the permitToken user', async () => {
    ...
    const { body, statusCode } = await request(server.getApp())
      .get(
        `/virtual-components/${virtualComponent2._id}/76fadvsdad87sad7as7g9i/triggers/name1`
      )
      .set('Authorization', 'permitToken');
  ...
  it('should return could not find the component Version to the permitToken user', async () => {
    ...
    const { body, statusCode } = await request(server.getApp())
      .get(
        `/virtual-components/${virtualComponent2._id}/${validId}/triggers/name3`
      )
      .set('Authorization', 'permitToken');
  ...
  it('should return could not find the trigger to the permitToken user', async () => {
    ...
    const { body, statusCode } = await request(server.getApp())
      .get(
        `/virtual-components/${virtualComponent2._id}/${componentVersion2._id}/triggers/nam310`
      )
      .set('Authorization', 'permitToken');
