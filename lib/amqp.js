...
function AMQPConnection(settings) {
    ...
    function sendDataToQueue(queueName, payload, options, encrypt) {
        ...
        try {
            self.publishChannel.sendToQueue(queueName, content, options);
        } catch (err) {
            console.log('Failed to send message to queue ' + queueName);
        }
    }
    ...
    this.disconnect = function disconnect(){
        ...
        try {
            ...
            debug('Subscribe channel is closed already');
        }
        try {
            self.publishChannel.close();
        } catch (alreadyClosed) {
            debug('Publish channel is closed already');
        }
        try {
            self.amqp.close();
        } catch (alreadyClosed) {
            debug('AMQP connection is closed already');
        }
        debug('Successfully closed AMQP connections');
