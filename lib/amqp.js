...
var AMQPConnection = function(settings) {
    ...
    self.processData = function processData(data, originalMessage){
    ...
    self.processError = function processError(err, originalMessage){
        var errorData = {
            message: err.message,
            stack: err.stack
        };
    ...
    self.processRebound = function processRebound(reboundError, originalMessage){
        ...
        if (options.headers.reboundIteration > settings.REBOUND_LIMIT) {
            processError(new Error('Rebound limit exceeded'), originalMessage);
        } else {
