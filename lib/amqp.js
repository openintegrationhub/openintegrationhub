...
AMQPConnection.prototype.sendData = function sendData(data, originalMessage) {
    var settings = this.settings;
    var options = {
        contentType: 'application/json',
        contentEncoding: 'utf8',
        mandatory: true,
        headers: _.omit(originalMessage.properties.headers, 'x-death')
    };
    this.sendToExchange(settings.PUBLISH_MESSAGES_TO, settings.DATA_ROUTING_KEY, data, options, true);
};

AMQPConnection.prototype.sendError = function sendError(err, originalMessage) {
    var settings = this.settings;
    var options = {
        contentType: 'application/json',
        contentEncoding: 'utf8',
...
AMQPConnection.prototype.sendRebound = function sendRebound(reboundError, originalMessage) {
    var settings = this.settings;
    ...
    if (options.headers.reboundIteration > settings.REBOUND_LIMIT) {
        this.sendError(new Error('Rebound limit exceeded'), originalMessage);
    } else {
