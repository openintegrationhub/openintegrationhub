...
AMQPConnection.prototype.listenQueue = function listenQueue(queueName, callback) {
    ...
    var prefetch = this.settings.RABBITMQ_PREFETCH_SAILOR || 1;

    this.subscribeChannel.prefetch(parseInt(prefetch));
    ...
    return this.subscribeChannel.consume(queueName, function decryptMessage(message){
        ...
        try {
            var decryptedContent = encryptor.decryptMessageContent(message.content);
        } catch (err) {
...
AMQPConnection.prototype.sendData = function sendData(data, headers) {
    ...
    var encryptedData = encryptor.encryptMessageContent(data);
...
AMQPConnection.prototype.sendError = function sendError(err, headers, originalMessageContent) {
    ...
    var options = getOptions(headers);
    var payload = {};
    payload.error = encryptor.encryptMessageContent({
        name: err.name,
        message: err.message,
        stack: err.stack
    });
