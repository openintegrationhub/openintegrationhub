...
class Amqp {
    ...
    listenQueue(queueName, callback) {
        return co(function* doListenQueue() {
            if (this.consumerTag) {
            ...
            const result = yield this.subscribeChannel.consume(queueName, this.decryptMessage.bind(this, callback));
    ...
    listenQueueCancel() {
        return co(function* doCancel() {
            ...
            yield this.subscribeChannel.cancel(this.consumerTag);
            this.consumerTag = null;
    ...
    async publishMessage(exchangeName, routingKey, payloadBuffer, options, iteration) {
        ...
        const promisifiedPublish = async (exchangeName, routingKey, payloadBuffer, options) => {
            let result;
