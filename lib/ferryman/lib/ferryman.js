...
class Ferryman {
    ...
    async createDataHubEntry(applicationUid, recordUid, token) {
        try {
            const body = {
                domainId: '',
                schemaUri: '',
    ...
    // eslint-disable-next-line consistent-this
      ...
      || !message.properties.headers.orchestratorToken
        ...
        this.oihMeta = (typeof (message.metadata) === 'object') ? message.metadata : {};

        let hasOihUid = false;
        // Check if oihUid exists
        if ('id' in this.oihMeta || this.oihMeta.id) {
            const data = await this.getDataHubEntryByOihId(this.oihMeta.id, tokenData.apiKey);

            if (data && data.id) {
                this.oihMeta.id = data.id;
                this.oihMeta.refs = data.refs;
                hasOihUid = true;
            }
        // Check if recordUid exists
        } else if ('recordUid' in this.oihMeta || this.oihMeta.recordUid) {
            const data = await this.getDataHubEntryByRecordId(this.oihMeta.recordUid, tokenData.apiKey);
        ...
        if (hasOihUid === false) {
            ...
            const applicationUid = this.oihMeta.applicationUid;
            const recordUid = this.oihMeta.recordUid;
            const data = await this.createDataHubEntry(applicationUid, recordUid, tokenData.apiKey);
            if (data && data.id) {
                this.oihMeta.id = data.id;
                this.oihMeta.refs = [{
                    applicationUid,
        ...
        if (this.oihMeta.applicationUid && this.oihMeta.refs && Array.isArray(this.oihMeta.refs)) {
            let ref;
            for (ref in this.oihMeta.refs) {
                if (ref.applicationUid === this.oihMeta.applicationUid) {
        ...
        if (tokenData.secretId && tokenData.apiKey) {
