...
class Ferryman {
    ...
    async getIdMeta(oihMeta, self) {
        ...
        if ('id' in oihMeta || oihMeta.id) {
            const data = await self.getDataHubEntryByOihId(oihMeta.id, self.apiKey);
        ...
        if (this.settings.DATAHUB_BASE_URL) {
            if (cfg.nodeSettings.idLinking) {
                getIdMetaResult = await this.getIdMeta({
                    id: self.oihMeta.id,
                    recordUid: self.oihMeta.recordUid
                }, self);
            }
        } else {
        ...
        return new Promise((resolve) => {
            ...
            const onData = async (data) => {
                ...
                if (this.settings.DATAHUB_BASE_URL && cfg.nodeSettings.idLinking) {
                    ...
                    const getIdMetaResult = this.getIdMeta({ recordUid: data.metadata.recordUid }, self);
