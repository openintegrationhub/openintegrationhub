...
class Ferryman {
    ...
    async deleteRecordIdFromDataHubEntry(oihUid, recordUid, token) {
        try {
            const options = {
                uri: `${this.settings.DATAHUB_BASE_URL}/data/${oihUid}/${recordUid}`,
                method: 'DELETE',
                headers: {
                    'accept': 'application/json',
                    'Content-Type': 'application/json',
            ...
            log.warn(`Failed to post entry to the data-hub via oihUid ${oihUid} recordId ${recordUid}`);

            return null;
        } catch (e) {
            log.error(e);
            // eslint-disable-next-line max-len
            log.warn(`Error: Failed to post entry to the data-hub via oihUid ${oihUid} recordId ${recordUid}`);
    ...
    async upsertDataHubEntry(applicationUid, recordUid, oihUid, token) {
        try {
            ...
            log.warn(`Failed to post entry to the data-hub via oihUid ${applicationUid} recordId ${recordUid}`);

            return null;
        } catch (e) {
            log.error(e);
            // eslint-disable-next-line max-len
            log.warn(`Error: Failed to post entry to the data-hub via oihUid ${applicationUid} recordId ${recordUid}`);
            ...
            const onData = async (data) => {
                ...
                const currentApplicationUid = (
                  ...
                  && passedCfg.nodeSettings.applicationUid
                    ...
                    : ((data && data.metatdata) ? data.metadata.applicationUid : null);
                ...
                try {
                    ...
                    if (
                      ...
                      && passedCfg.nodeSettings.governance
                        ...
                        if (passedCfg.fields && passedCfg.deletes && data.data.delete) {
                            deleteType = data.data.delete;

                            if (data.data.delete === 'confirmed') {
                                const response = await self.deleteRecordIdFromDataHubEntry(
                                    data.metadata.oihUid,
                                    data.metadata.recordUid,
                                    self.apiKey
                                );

                                if (response === false) {
                                    // eslint-disable-next-line
                                    log.error(`DataHub update failed. RecordUid: ${data.metadata.recordUid} not found in entry: ${data.metadata.oihUid}`);
                                } else if (response === null) {
                                    // eslint-disable-next-line
                                    log.error(`DataHub update failed for recordUid: ${data.metadata.recordUid} oihUid: ${data.metadata.oihUid}`);
                                }
                            }
                        }
