      ...
      || !message.properties.headers.orchestratorToken
        ...
        if (this.settings.DATAHUB_BASE_URL) {
            if (cfg.nodeSettings.idLinking) {
                ...
                getIdMetaResult = await this.getIdMeta(self);
                log.debug('Result: ', JSON.stringify(getIdMetaResult));
                if (getIdMetaResult.oihUid) {self.oihUid = getIdMetaResult.oihUid;}
            }
        } else {
        ...
        return new Promise((resolve) => {
            ...
            const onData = async (data) => {
                ...
                if (this.settings.DATAHUB_BASE_URL && cfg.nodeSettings.idLinking) {
                    ...
                    const getIdMetaResult = this.getIdMeta(self);
                    ...
                    if (dataHubEntryHasRecordUid === false) {
                        ...
                        const response = await this.upsertDataHubEntry(applicationUid, recordUid, oihUid, this.apiKey);
                        if (response && response.id && data.metadata) {
                            console.log('Created DH entry:', JSON.stringify(response));
                            self.oihMeta.oihUid = response.id;
                        }
                    } else {
                        self.oihMeta.oihUid = getIdMetaResult.oihUid;
                    }
