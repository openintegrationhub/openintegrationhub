...
class Ferryman {
    ...
    async processMessage(payload, message) {
        log.info('payload.metadata', JSON.stringify(payload.metadata));
      ...
      || !message.properties.headers.orchestratorToken
        ...
        if (tokenData) {
            ...
            log.info('this.nodeSettings', this.nodeSettings);
        ...
        if (this.settings.DATAHUB_BASE_URL) {
            if (cfg.nodeSettings.idLinking) {
                log.info('ID Linking Process Message!');
                log.info('oihUid: ', payloadOihMeta.oihUid);
                log.info('recordUid: ', payloadOihMeta.recordUid);
                getIdMetaResult = await this.getIdMeta({
                    oihUid: payloadOihMeta.oihUid,
                    recordUid: payloadOihMeta.recordUid
                }, self);
                log.info('getIDMetaResult: ', JSON.stringify(getIdMetaResult));


                // Check if we have any recordUid to pass to component
                log.info('Looking for own ref');
                log.info('applicationUid:', this.nodeSettings.applicationUid);
                if (getIdMetaResult && getIdMetaResult.refs && Array.isArray(getIdMetaResult.refs)) {
                    for (let ref of getIdMetaResult.refs) {
                        if (ref.applicationUid === this.nodeSettings.applicationUid) {
                            ...
                            log.info('Found own Ref!');
                            log.info('recordUid: ', ref.recordUid);
                            break;
                        }
                    }
                }
        ...
        return new Promise((resolve) => {
            ...
            const onData = async (data) => {
                log.info('data.metadata', JSON.stringify(data.metadata));
                log.info('data', JSON.stringify(data));
                log.info('tokenData.nodeSettings', JSON.stringify(cfg.nodeSettings));
                        ...
                        for (let ref of getIdMetaResult.refs) {
                            if (ref.applicationUid === cfg.nodeSettings.applicationUid) {
                                if (ref.recordUid === data.metadata.recordUid) {
                                    log.info('Found own ref: ', ref.recordUid);
