...
class Ferryman {
    ...
    async addIdMeta(self) {
        console.log('addIdMeta', JSON.stringify(self.oihMeta));
        let oihUid = false;
        let refs = false;
    ...
    // eslint-disable-next-line consistent-this
      ...
      || !message.properties.headers.orchestratorToken
        ...
        let addIdMetaResult = false;
        if (this.settings.DATAHUB_BASE_URL) {
            if (cfg.nodeSettings.idLinking) {
                log.debug('About to fetch id linking data for incoming data');
                addIdMetaResult = await this.addIdMeta(self);
                log.debug('Result: ', JSON.stringify(addIdMetaResult))
                if (addIdMetaResult.oihUid) {self.oihUid = addIdMetaResult.oihUid;}
            }
        } else {
        ...
        //     }
        // }
        if (addIdMetaResult && addIdMetaResult.refs && Array.isArray(addIdMetaResult.refs)) {
            for (let ref of addIdMetaResult.refs) {
                if (ref.applicationUid === this.nodeSettings.applicationUid) {
                    // this.oihMeta.recordUid = ref.recordUid;
                    self.oihMeta.recordUid = ref.recordUid;
                    payload.metadata.recordUid = ref.recordUid;
                    break;
                }
            }
        ...
        return new Promise((resolve) => {
            ...
            const onData = async (data) => {
                ...
                if (this.settings.DATAHUB_BASE_URL && cfg.nodeSettings.idLinking) {
                    ...
                    if (data.metadata && data.metadata.recordUid) {self.recordUid = data.metadata.recordUid;}
                    const addIdMetaResult = this.addIdMeta(self);

                    if (addIdMetaResult.oihUid === false) {
                        // Create new oihUid if none found
                        // const { recordUid, oihUid } = this.oihMeta;
                        const { recordUid } = data.metadata;
                        const oihUid = (self.oihMeta && self.oihMeta.oihUid) ? self.oihMeta.oihUid : null;
                        ...
                        if (response && response.id && data.metadata) {
                            ...
                            console.log('Created DH entry:', JSON.stringify(response));
                            self.oihMeta.oihUid = response.id;
                        }
                    } else {
                        self.oihMeta.oihUid = addIdMetaResult.oihUid;
                    }

                    // if (this.oihMeta && this.oihMeta.id) {
                    //     data.metadata.oihUid = this.oihMeta.id;
                    // }
                    if (this.oihMeta && this.oihMeta.oihUid) {
                        data.metadata.oihUid = this.oihMeta.oihUid;
                    }
