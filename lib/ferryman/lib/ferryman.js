...
class Ferryman {
    ...
    async getDataHubEntryByOihId(oihUid, token) {
        log.warn('getDataHubEntryByOihId', oihUid, token);
        try {
            const getOptions = {
    ...
    async getDataHubEntryByRecordId(recordUid, token) {
        log.warn('getDataHubEntryByRecordId', recordUid, token);
        try {
            const getOptions = {
    ...
    async getIdMeta(oihMeta, self) {
        log.warn('getIdMeta', JSON.stringify(oihMeta));
        let oihUid = false;
        let refs = false;
    ...
    async processMessage(payload, message) {
      ...
      || !message.properties.headers.orchestratorToken
        ...
        if (tokenData) {
            ...
            console.log('this.nodeSettings', this.nodeSettings);
        ...
        if (this.settings.DATAHUB_BASE_URL) {
            if (cfg.nodeSettings.idLinking) {
                console.log('ID Linking Process Message!');
                console.log('oihUid: ', payloadOihMeta.oihUid);
                console.log('recordUid: ', payloadOihMeta.recordUid);
                getIdMetaResult = await this.getIdMeta({
                    oihUid: payloadOihMeta.oihUid,
                    recordUid: payloadOihMeta.recordUid
                }, self);
                console.log('getIDMetaResult: ', JSON.stringify(getIdMetaResult));


                // Check if we have any recordUid to pass to component
                console.log('Looking for own ref');
                console.log('applicationUid:', this.nodeSettings.applicationUid);
                if (getIdMetaResult && getIdMetaResult.refs && Array.isArray(getIdMetaResult.refs)) {
                    for (let ref of getIdMetaResult.refs) {
                        if (ref.applicationUid === this.nodeSettings.applicationUid) {
                            ...
                            console.log('Found own Ref!');
                            console.log('recordUid: ', ref.recordUid);
                            break;
                        }
                    }
                }
        ...
        return new Promise((resolve) => {
            ...
            const onData = async (data) => {
                console.log('data.metadata', JSON.stringify(data.metadata));
                console.log('data', JSON.stringify(data));
                console.log('tokenData.nodeSettings', JSON.stringify(cfg.nodeSettings));
                ...
                if (cfg.nodeSettings && cfg.nodeSettings.applyTransform
                    && (cfg.nodeSettings.applyTransform === 'after'
                      || cfg.nodeSettings.applyTransform === 'both')
                ) {
                    const transformCfg = {
                        customMapping: cfg.nodeSettings.secondTransformFunction
                            ? cfg.nodeSettings.secondTransformFunction : cfg.nodeSettings.transformFunction
                    };
                    const transformedMsg = transform(message, transformCfg, false);
                    data = transformedMsg;
                }
                ...
                if (self.settings.DATAHUB_BASE_URL && cfg.nodeSettings.idLinking) {
                    console.log('In onData ID Linking!');
                    console.log('oihUid:', data.metadata.oihUid);
                    console.log('recordUid:', data.metadata.recordUid);
                    ...
                    if (getIdMetaResult && getIdMetaResult.refs && Array.isArray(getIdMetaResult.refs)) {
                        for (let ref of getIdMetaResult.refs) {
                            if (ref.applicationUid === cfg.nodeSettings.applicationUid) {
                                if (ref.recordUid === data.metadata.recordUid) {
                                    console.log('Found own ref: ', ref.recordUid);
                    ...
                    if (dataHubEntryHasRecordUid === false) {
                        ...
                        const applicationUid = cfg.nodeSettings.applicationUid || self.oihMeta.applicationUid;

                        console.log('About to upsert data entry with: ');
                        console.log('applicationUid: ', applicationUid);
                        console.log('recordUid: ', recordUid);
                        console.log('oihUid: ', oihUid);
                        const response = await self.upsertDataHubEntry(applicationUid, recordUid, oihUid, self.apiKey);
                ...
                try {
                    ...
                    if (
                        cfg.nodeSettings
                      && typeof (cfg.nodeSettings === 'object')
                      && cfg.nodeSettings.governance
                    ) {
