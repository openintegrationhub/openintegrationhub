...
class Ferryman {
    ...
    async deleteRecordIdFromDataHubEntry(oihUid, recordUid, token) {
        try {
            ...
            log.warn(`Failed to post entry to the data-hub via applicationUid ${oihUid} recordId ${recordUid}`);

            return null;
        } catch (e) {
            log.error(e);
            // eslint-disable-next-line max-len
            log.warn(`Error: Failed to post entry to the data-hub via applicationUid ${oihUid} recordId ${recordUid}`);
