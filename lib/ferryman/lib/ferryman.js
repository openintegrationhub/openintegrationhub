...
class Ferryman {
    ...
    // eslint-disable-next-line consistent-this
      ...
      || !message.properties.headers.orchestratorToken
        ...
        this.oihMeta = (typeof (message.properties.payload.metadata) === 'object')
            ? message.properties.payload.metadata : {};

        if (this.settings.DATAHUB_BASE_URL) {
            let hasOihUid = false;
            // Check if oihUid exists
            if ('id' in this.oihMeta || this.oihMeta.id) {
                const data = await this.getDataHubEntryByOihId(this.oihMeta.id, this.apiKey);
                ...
                const data = await this.getDataHubEntryByRecordId(this.oihMeta.recordUid, this.apiKey);
            ...
            if (hasOihUid === false) {
                ...
                const data = await this.createDataHubEntry(applicationUid, recordUid, this.apiKey);
                if (data && data.id) {
                    this.oihMeta.id = data.id;
                    this.oihMeta.refs = [{
                        applicationUid,
