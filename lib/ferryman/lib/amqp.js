...
class Amqp {
    ...
    async connect(uri) {
        ...
        if (this.isConnected) {
            ...
            if (!this.hasSubscribeChannel) {
                ...
                if (!this.hasSubscribeChannel) {
                    log.info('Not connected to subscribe channel retrying in 2s');
                    setTimeout((uri)=>{ this.connect(uri); }, 2000, uri);
                }
            ...
            if (!this.hasPublishChannel) {
                log.info('Not connected to publish channel retrying in 2s');
                setTimeout((uri)=>{ this.connect(uri); }, 2000, uri);
            }
        } else {
            log.info('Not connected retrying in 2s');
            setTimeout((uri)=>{ this.connect(uri); }, 2000, uri);
        }
    }
    ...
    async sendRawRecord(data, headers, throttle) {
        const { payload, rawRecordId } = data;
        const properties = this._createPropsFromHeaders(headers);
        const { settings } = this;
        ...
        return this.sendToExchange(settings.EVENT_BUS_EXCHANGE, routingKey, encryptedData, properties, throttle);
    }
