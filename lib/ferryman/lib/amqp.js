...
class Amqp {
    ...
    async disconnect() {
        ...
        if (this.subscribeChannel.removeAllListeners) {
            this.subscribeChannel.removeAllListeners('close');
        }
        if (this.publishChannel.removeAllListeners) {
    ...
    decryptMessage(callback, message) {
        ...
        if (message === null) {
            ...
            log.criticalErrorAndExit('RabbitMQ channel is cancelled, going to die...');
            return;
        }
