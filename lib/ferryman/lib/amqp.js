...
let latestActiveConnection = null;
class Amqp {
    ...
    async connect(uri) {
        ...
        latestActiveConnection = this.amqp;
    ...
    async sendRawDataChannel(data, headers, throttle) {
        const properties = this._createPropsFromHeaders(headers);
        const { settings } = this;

        const routingKey = getRoutingKeyFromHeaders(data.headers) || settings.RDS_ROUTING_KEY;
        properties.headers.protocolVersion = settings.PROTOCOL_VERSION;
...
module.exports.latestActiveConnection = latestActiveConnection;
