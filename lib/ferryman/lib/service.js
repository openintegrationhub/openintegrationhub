...
class ServiceExec extends EventEmitter {
    constructor({ logger, services }) {
        ...
        assert(this._services.apiClient,
            'ServiceExec should be created with api client'); // eslint-disable-line no-underscore-dangle
        assert(this._services.config,
            'ServiceExec should be created with config'); // eslint-disable-line no-underscore-dangle
    }
