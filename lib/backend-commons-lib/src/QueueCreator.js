...
class QueueCreator {
     ...
     */
    async makeQueuesForTheFlow(flow, componentsMap) {
    ...
    async prepareExchangeForGlobalComponent(component) {
        const exchange = `component-${component.id}`;
    ...
    async createQueuesForGlobalComponent(component) {
        const exchangeName = await this.prepareExchangeForGlobalComponent(component)
        const messagesQueue = `${exchangeName}:messages`
        const reboundsQueue = `${exchangeName}:rebounds`

        const inputRoutingKey = `${exchangeName}.input`
        const reboundRoutingKey = `${exchangeName}.rebound`
        const requeueRoutingKey = `${exchangeName}.requeue`
        // const snapshotRoutingKey = `${exchangeName}.snapshot`
        const errorRoutingKey = `${exchangeName}.error`
        const deadLetterRoutingKey = `${exchangeName}.deadletter`
