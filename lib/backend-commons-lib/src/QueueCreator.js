...
function getOutputRoutingKey(flow, stepId) {
    const suffix = getRoutingKeySuffixForFlow(flow, OUTPUT_TAG);
...
class QueueCreator {
    ...
    getAmqpStepConfig(flow, stepId, isLastNode) {
        return {
            ...
            outputRoutingKey: getOutputRoutingKey(flow, stepId),
     ...
     * @returns {QueueSteps}
     */
    async createQueuesForFlowNode(flow, node, parentNode, isLastNode) {
        ...
        } = this.getAmqpStepConfig(flow, stepId, isLastNode);
        ...
        if (parentNode) {
            const parentNodeOutputRoutingKey = getOutputRoutingKey(flow, parentNode.id);
            await this.bindQueue(messagesQueue, exchangeName, parentNodeOutputRoutingKey);
        }
        ...
        console.log({
            ...
            OUTPUT_ROUTING_KEY: outputRoutingKey
        })
        return {
            LISTEN_MESSAGES_ON: messagesQueue,
            PUBLISH_MESSAGES_TO: exchangeName,
