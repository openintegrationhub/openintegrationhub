...
ComponentReader.prototype.loadTriggerOrAction = function loadTriggerOrAction(name) {
    var filename, modulePath, result;
    ...
    try {
        ...
        err.message = util.format(
            "Trigger or action '%s' is not found. " +
            "Please check if path you specified in component.json ('%s') is valid.",
            name, filename
        );
        debug(err);
        return Q.reject(err);
...
ComponentReader.prototype.loadVerifyCredentials = function loadVerifyCredentials() {
    ...
    try {
        var filePath = path.join(this.componentPath, 'verifyCredentials');
        debug('Trying to find verifyCredentials.js at: %s', filePath);
