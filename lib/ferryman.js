...
class Ferryman {
    ...
    async getSnapShot(flowId, stepId) {
        const getOptions = {
            ...
            headers: {
                ...
                'headers': {
                    ...
                    // @todo: take dynamically from events
                }
            },
            json: true,
    ...
    async processMessage(payload, message) {
        ...
        let secret = false;
        if (message.properties.headers.secret && message.properties.headers.authToken) {
            secret = await this.fetchSecret(message.properties.headers.secret, message.properties.headers.authToken);
        }
