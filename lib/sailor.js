...
class Sailor {
    ...
    startup() {
        return co(function* doStartup() {
            ...
            try {
                if (!result) {
                    console.warn('Startup hook has not returned any data. Deleting existing data.');
                    yield handle.delete();
                    return;
                }
                ...
                if (e.statusCode === 409) {
                    console.warn('Hooks data creating conflict. Deleting existing data');
                    yield handle.delete();
                    yield handle.create(result);
                }
            }
    ...
    shutdown() {
        return co(function* doShutdown() {
            ...
            const handle = hooksData.startup(this.settings);
            try {
                startupData = yield handle.retrieve();
            } catch (e) {
                console.error('Can not retrieve startup data', _.pick(e, ['message', 'statusCode', 'name']));
                throw e;
            }
            yield this.invokeModuleFunction('shutdown', startupData);
            startupData = yield handle.delete();
        }.bind(this));
    }
