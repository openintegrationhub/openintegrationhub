...
Sailor.prototype.prepare = function prepare() {
    ...
    function getStepData() {
        ...
        const taskId = self.settings.FLOW_ID;
...
Sailor.prototype.reportError = function reportError(err) {
    const headers = {
...
Sailor.prototype.startup = function onFlowStart() {
...
Sailor.prototype.shutdown = function shutdown() {
    return co(function* gen() {
...
Sailor.prototype.readIncomingMessageHeaders = function readIncomingMessageHeaders(message) {
    ...
    if (props.messageId) {
        result.parentMessageId = props.messageId;
    }
...
Sailor.prototype.processMessage = function processMessage(payload, message) {
    ...
    console.log('PROPS:%j', message.properties);
    console.log('CorID:', message.properties.correlationId);
    ...
    function createAmqpProperties(headers, messageId) {
        ...
        const props = message.properties;
        const result = createDefaultAmqpProperties(headers);

        result.messageId =  messageId? messageId: uuid.v1();
