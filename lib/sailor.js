...
Sailor.prototype.connect = function connect(){
    ...
    var mongoConnected = this.mongoConnection.connect(this.settings.MONGO_URI);
    var amqpConnected = this.amqpConnection.connect(this.settings.AMQP_URI);
    var componentFound = this.componentReader.init(this.settings.COMPONENT_PATH);
