...
Sailor.prototype.processMessage = function processMessage(payload, message) {
    ...
    function processMessageWithModule(module) {
        ...
        function onData(data) {
            ...
            return promise(sailor.amqpConnection.sendData(data, headers));
        }
        ...
        function onError(err) {
            ...
            headers.end = new Date().getTime();
            return promise(sailor.amqpConnection.sendError(err, headers, message.content));
        }
        ...
        function onRebound(err) {
            ...
            return promise(sailor.amqpConnection.sendRebound(err, message, headers));
        }
        ...
        function onSnapshot(data) {
            ...
            return promise(sailor.amqpConnection.sendSnapshot(data, headers));
        }

        function onUpdateKeys(keys) {
            info('Message #%s updateKeys emitted', message.fields.deliveryTag);

            return promise(sailor.apiClient.updateKeys(cfg._account, keys)
        ...
        function onEnd() {
            ...
            Q.allSettled(subPromises).then(resolveEndPromise);
        }

        function resolveEndPromise(){
            executionEnd.resolve();
        }

        return executionEnd.promise;
    }
