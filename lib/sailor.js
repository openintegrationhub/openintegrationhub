...
class Sailor {
    ...
    startup() {
        return co(function* doStartup() {
            log.info('hook-startup begin');
            const result = yield this.invokeModuleFunction('startup');
            log.debug('hook-startup result', { result });
            ...
            try {
                ...
                if (e.statusCode === 409) {
                    log.warn('hook-startup – hooks data creating conflict. Deleting existing data');
                    yield handle.delete();
                    yield handle.create(result);
                } else {
                    log.warn('hook-startup – error');
                    throw e;
                }
            }
            log.info('hook-startup finished');
            return result;
        }.bind(this));
    }
    ...
    shutdown() {
        return co(function* doShutdown() {
            ...
            yield handle.delete();
            log.info('hook-shutdown finished');
        }.bind(this));
    }

    init() {
        return co(function* doInit() {
            log.info('hook-init start');
            yield this.invokeModuleFunction('init');
            log.info('hook-init finished');
        }.bind(this));
    }
