...
Sailor.prototype.processMessage = function processMessage(message, ack, rebound) {
    ...
    return sailor.componentReader.findTriggerOrAction(step.function).then(function(module){
        ...
        taskExec.on("data", function processData(data){
            sailor.amqpConnection.sendToQueue(settings.OUTGOING_MESSAGES_QUEUE.name, data);
        });

        taskExec.on("error", function processError(err){
            var errorData = {
                message: err.message,
                stack: err.stack
            };
            sailor.amqpConnection.sendToQueue(settings.ERRORS_QUEUE.name, errorData);
        });
