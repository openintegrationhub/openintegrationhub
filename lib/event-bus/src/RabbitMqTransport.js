...
class RabbitMqTransport extends Transport {
        ...
        try {
            if (!this._publishChannel) {
                ...
                if (this._publishChannel.connection.stream._hadError) {
                    this._logger.error(this._publishChannel.connection.stream, 'Channel error');
                }
            }
    ...
    async _handleSubscription({topics, serviceName}, callback) {
        ...
        return channel.consume(queueName, async msg => {
            ...
            // for special case when msg = null is coming on chanel close
            if (!msg || !msg.content) {
                return;
            }
            ...
            try {
                ...
                return this._logger.error({err, msg: msg.content}, 'Failed to parse message content');
            }
