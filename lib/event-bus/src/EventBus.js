...
class EventBus {
    constructor({ transport, serviceName, logger, rabbitmqUri, onCloseCallback, onErrorCallback, onReconnectCallback }) {
        ...
        if (!transport) {
            if (rabbitmqUri) {
                ...
                transport = new RabbitMqTransport({ rabbitmqUri, logger, onCloseCallback, onErrorCallback, onReconnectCallback });
            } else {
    ...
    async _handleEvent(event) {
        ...
        if (!eventName) {
            await event.ack();
            return this._logger.trace({ event }, 'Missing event name in header');
        }
    ...
    getConnection() {
        return this._transport._connection
    }

    setOnCloseCallback(onCloseCallback){
        this._transport.onCloseCallback = onCloseCallback
    }

    setOnErrorCallback(onErrorCallback){
        this._transport.onErrorCallback = onErrorCallback
    }

    setOnReconnectCallback(onReconnectCallback){
        this._transport.onReconnectCallback = onReconnectCallback
    }
}
