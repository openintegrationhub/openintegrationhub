...
class EventBus {
    constructor({ transport, serviceName, logger, rabbitmqUri, onCloseCallback }) {
        ...
        if (!transport) {
            if (rabbitmqUri) {
                ...
                transport = new RabbitMqTransport({ rabbitmqUri, logger, onCloseCallback });
            } else {
    ...
    updateOnCloseCallback(onCloseCallback){
        this._transport.updateOnCloseCallback(onCloseCallback)
    }
}
