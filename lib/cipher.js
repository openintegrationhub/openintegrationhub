...
function encrypt(rawData) {
    if (!_.isString(rawData)) {
        throw new Error('RabbitMQ message cipher.encrypt() accepts only string as parameter.');
    }
...
function decrypt(encData) {
    if (!_.isString(encData)) {
        throw new Error('RabbitMQ message cipher.decrypt() accepts only string as parameter.');
    }
    if (!PASSWORD) {
