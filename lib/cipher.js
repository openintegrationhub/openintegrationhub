...
function decryptIV(encData, options) {
    ...
    var skipUriDecode = options[featureFlags.SKIP_MESSAGE_URL_DECODING] !== undefined;
    ...
    if (!PASSWORD) {
        return skipUriDecode ? encData : decodeURIComponent(encData);
    }
    ...
    var result = cipher.update(encData, 'base64', 'binary') + cipher.final('binary');

    return skipUriDecode ? result : decodeURIComponent(result);
}
