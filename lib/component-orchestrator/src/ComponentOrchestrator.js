...
class ComponentOrchestrator {
    ...
    async _verifyAndPrepareRunNextSteps(flow, targetFlowStep) {
        const [ targetFlowId, targetStepId ] = targetFlowStep.split(':')
    ...
    async _handleMessage(message) {
        try {
            ...
            const { flowExecId, orchestratorToken } = message.properties.headers;
            const { stepId, flowId, apiKey, specialFlags } = jwt.verify(orchestratorToken, this._tokenSecret);
