...
class ComponentOrchestrator {
    ...
    async _processState(allFlows, allApps) {
        ...
        for (let flow of allFlows) {
            try {
                await this._handleFlowState(flow, appsIndex, componentsCache);
            } catch (err) {
    ...
    async _handleFlowState(flow, appsIndex, componentsCache = new Map()) {
        ...
        for (let node of nodes) {
            if (!appsIndex[flow.id] || !appsIndex[flow.id][node.id]) {
                ...
                for (let node of nodes) {
                    if (!componentsCache.get(node.componentId)) {
                        const component = await this._componentsDao.findById(node.componentId)
                        componentsCache.set(node.componentId, component)
                    }
                }
                ...
                const component = componentsCache.get(node.componentId);
    ...
    async _executeStep({
        ...
        console.log('#### orchestrator', exchangeName, messagesQueue)
