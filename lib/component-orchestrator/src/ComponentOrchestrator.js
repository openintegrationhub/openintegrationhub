...
class ComponentOrchestrator {
    constructor({
        ...
        this._amqpChannelWrapper = amqpChannelWrapper;
    }

    async start() {
        this.setupAMPQRecover()
    }

    setupAMPQRecover() {
        // this._eventBus.setOnReconnectCallback(() => console.log('fooooobar'))

        this._amqpChannelWrapper.addSetup( async (channel) => {
            try {

                // ensure loop is stopped
                stopLoop();

                this._queuesManager.onRecover(channel)
                ...
                const { flowNodes, globalComponents } = await this._queuesManager.getExistingCredentialIdentifiers()

                this._logger.debug({ flowNodes, globalComponents }, 'Will recover credentials')

                // ensure credentials and queues step by step to prevent dos
                for (const flowNode of flowNodes) {
                    // recover flow nodes
                    const [flowId, nodeId] = flowNode.split('.')
                    await this._queuesManager.ensureCredentials(null, { id: flowId }, { id: nodeId })
                    ...
                    for (let node of nodes) {
                        ...
                        await this._queuesManager.prepareQueues(
                            flow,
                            components
                        );
                ...
                for (const componentId of globalComponents) {
                    // recover global components
                    await this._queuesManager.ensureCredentials({ id: componentId }, null, null)
    ...
    async _handleFlowState(flow, appsIndex) {
        ...
        for (let node of nodes) {
            // if flow node is already running
