...
class ComponentOrchestrator {
        ...
        for (let node of nodes) {
            if (!appsIndex[flow.id] || !appsIndex[flow.id][node.id]) {
                ...
                if (appsIndex[node.componentId]) {
                    logger.trace({ nodeId: node.id, componentId: node.componentId }, 'Found global component. Skipping deployment');
                    continue
                }
                ...
                if (!componentsMap) {
                    ...
                    for (let node of nodes) {
                        componentPromises.push(this._componentsDao.findById(node.componentId).then(component => componentsMap.set(node.id, component)))
                    }
                ...
                if (component.isGlobal) {
                    logger.trace({ componentId: node.componentId }, 'Skip global component');
                    continue;
                }

                logger.trace({ nodeId: node.id }, 'Going to deploy a flow node');

                const flowSettings = await this._queuesManager.prepareQueues(flow, componentsMap);
                ...
                await this._driver.createApp({
                    flow, node, envVars, component, options: {
                        replicas: 1,
        ...
        try {
            ...
            if (nextSteps.length) {
                const promises = nextSteps.map(stepId => {
                    const componentId = flow.getComponentPropertyByStep(stepId, 'componentId');
                    return this._componentsDao.findById(componentId).then(component => this._executeStep(flow, stepId, flow.getComponentPropertyByStep(stepId, 'function'), component, token))
                })
    ...
    async _executeStep(flow, stepId, functionName, component, authToken) {
    ...
    async executeFlow(flow) {
        ...
        await this._executeStep(_flow, stepId, _flow.getComponentPropertyByStep(stepId, 'function'), component, token)
    }
    ...
    async stopComponent(component) {
        ...
        await this._driver.destroyApp({ name: `global-${component.id}`, type: 'global', componentId: component.id });
