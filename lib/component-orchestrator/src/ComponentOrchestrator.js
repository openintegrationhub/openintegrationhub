...
class ComponentOrchestrator {
    ...
    async _handleMessage(message) {
        try {
            ...
            if (nextSteps.length) {
                const promises = nextSteps.map(stepId => this._componentsDao.findById(stepId).then(component => this._executeStep(flow, stepId, component, token)))
    ...
    async _executeStep(flow, stepId, component, authToken) {

        const orchestratorToken = jwt.sign({
            flowId: flow._id.toString(), stepId
        }, SECRET);
    ...
    async executeFlow(flow) {
        ...
        await this._executeStep(_flow, stepId, component, token)
    }
