...
class ComponentOrchestrator {
        ...
        try {
            ...
            if (nextSteps.length) {
                const promises = nextSteps.map(stepId => {
                    ...
                    return this._componentsDao.findById(stepId)
                }).then(component => this._executeStep(flow, stepId, flow.getFunctionNameByStep(stepId), component, token)))
    ...
    async executeFlow(flow) {
        ...
        await this._executeStep(_flow, stepId, _flow.getFunctionNameByStep(stepId), component, token)
    }
