...
class ComponentOrchestrator {
    ...
    async _handleFlowState(flow, appsIndex, componentsCache = {}) {
        ...
        for (let node of nodes) {
            if (!appsIndex[flow.id] || !appsIndex[flow.id][node.id]) {
                ...
                for (let node of nodes) {
                    if (!componentsCache[node.componentId]) {
                        const component = await this._componentsDao.findById(node.componentId)
                        componentsCache[node.componentId] = component
                    }
                }
                ...
                const component = componentsCache[node.componentId];
