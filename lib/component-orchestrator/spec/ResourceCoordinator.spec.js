...
describe('ComponentOrchestrator', () => {
    ...
    describe('#_processState', () => {
        it('should process system state', async () => {
            ...
            expect(rc._handleFlowState.firstCall.args).to.deep.equal([flows[0], jobIndex, componentsCache]);
            expect(rc._handleFlowState.secondCall.args).to.deep.equal([flows[1], jobIndex, componentsCache]);
        });
    });
    ...
    describe('#_handleFlowState', () => {
        ...
        describe('for starting flow', () => {
            it('should ensure running nodes', async () => {
                ...
                //normal flow
                expect(queuesManager.getSettingsForNodeExecution).to.have.been.calledOnceWithExactly(flow, { id: 'step2' }, undefined);
