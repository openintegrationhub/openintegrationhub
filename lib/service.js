...
function getMetaModel(cfg, params) {
    function doGetMetaModel(module) {
        return Q.nfcall(module.getMetaModel, cfg);
    }

    return compReader.loadTriggerOrAction(params.triggerOrAction)
        .then(doGetMetaModel);
}

function selectModel(cfg, params) {
    function getModel(module) {
        assert(params.getModelMethod in module, 'Method not found');
        return Q.nfcall(module[params.getModelMethod], cfg);
    }
