...
describe('secrets', () => {
    ...
    test('Get all secrets', async () => {
        ...
        const resp = await request.post('/secrets')
            .set(...global.userAuth1)
            .send({
                data: {
                    name: 'string',
        ...
        const mixedRespBody = await request.get(`/secrets/${resp.body.data._id}`)
            .set(...global.userAuth1);
        expect(mixedRespBody.body.data.value.payload.nested.anotherval).toEqual(mixedSecret.nested.anotherval.replace(/./g, '*'));
