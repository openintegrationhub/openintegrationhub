...
const {
    SIMPLE, API_KEY, OA2_AUTHORIZATION_CODE, MIXED, SESSION_AUTH,
...
describe('secrets', () => {
    ...
    test('Session Auth setup and request', async () => {
        const example = nock('https://example.com');
        ...
        // fetch access-token
        const { body } = await request.get(`/secrets/${_id}`)
            .set(...global.userAuth1) //userToken1ExtraPerm
            .expect(200);
