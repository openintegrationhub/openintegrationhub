...
async function sessionRequest(url, {
    inputFields, authType, tokenPath,
}) {
    switch (authType) {
    case HEADER_AUTH:
        inputFields.forEach((entry) => addHeader(entry)); // TODO: Implement Header Building
        break;
    case BODY_AUTH:
        inputFields.forEach((entry) => addBody(entry)); // TODO: Implement Body Building
        break;
    case PARAMS_AUTH:
        inputFields.forEach((entry) => addParam(entry)); // TODO Implement Param Building
        break;
    default:
    }
    return new Promise((resolve, reject) => {
        request.post(url, {
            ...
            if (err) {
                ...
                try {
                    const obj = JSON.parse(body);
                    if (res.statusCode === 200) {
                        obj.parse(tokenPath); // TODO: Implement Response Parsing
                        resolve(obj);
                    } else {
                        reject(obj);
                    }
...
module.exports = {
    ...
    async refresh(authClient, secret) {
        ...
        case SESSION_AUTH: {
            ...
            const { authType, url } = authClient.endpoints.auth;
