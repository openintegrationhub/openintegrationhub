...
async function requestHelper(url, form) {
        ...
        form.entries((entry) => params.append(entry.key, inputFields(entry.key))); // TODO: Implement "smart" parsing of request fields
...
async function sessionRequest(url, {
    ...
    case PARAMS_AUTH:
        ...
        break;
    case BODY_AUTH:
        requestFields.forEach((entry) => addBody(entry)); // TODO: Implement "smart" parsing of request fields
        break;
    case HEADER_AUTH:
    ...
    try {
        const response = await fetch(url, { // TODO: pass params from above, then update response for fetch instead of request
            method: 'POST',
            body,
            headers,
        });
...
async function userinfoRequest(url, token) {
        fetch(url, { //TODO: Match these options to fetch options and check response
            headers: {
