...
// TODO: Implement variable replace
const parseHandlebars = (inputText) => {
...
async function sessionRequest(url, {
    ...
    case PARAMS_AUTH: {
        const params = new URLSearchParams();
        requestFields.forEach((entry) => params.append(entry.key, parseHandlebars(inputFields[entry.key])));
        body = params;
        break;
    }
    case BODY_AUTH:
        requestFields.forEach((entry) => body[entry.key] = parseHandlebars(inputFields[entry.key]));
        headers.append('Content-Type', 'application/json');
        body = JSON.stringify(body);
        break;
    case HEADER_AUTH:
        requestFields.forEach((entry) => headers.append(entry.key, parseHandlebars(inputFields[entry.key])));
        break;
    default:
    }
