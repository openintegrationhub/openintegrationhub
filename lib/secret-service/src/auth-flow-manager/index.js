...
async function sessionRequest(url, {
    authRequestConfig, requestFields, tokenPath,
}) {
    switch (authRequestConfig.type) {
    case HEADER_AUTH:
        requestFields;
        break;
    case BODY_AUTH:
        ...
        request.post(url, {
            ...
            if (err) {
                ...
                try {
                    const obj = JSON.parse(body);
                    if (res.statusCode === 200) {
                        obj.parse(tokenPath); // TODO: Implement
                        resolve(obj);
                    } else {
                        reject(obj);
                    }
...
module.exports = {
    ...
    async refresh(authClient, secret) {
        ...
        case SESSION_AUTH: {
            const { authRequestConfig, tokenPath } = authClient;
            const { requestFields } = secret.value;
            return await sessionRequest(authClient.endpoints.auth, {
                authRequestConfig,
                requestFields,
                tokenPath,
            });
