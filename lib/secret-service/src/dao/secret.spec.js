...
const { SIMPLE, OA2_AUTHORIZATION_CODE, MIXED, SESSION_AUTH } = require('../constant').AUTH_TYPE;
    ...
    test('cryptoSecret SESSION', async () => {
        ...
        let secret = SecretDAO.cryptoSecret({
            ...
            value: {
                authClientId: '5c0a4f796731613b7d10e73e',
                accessToken,
                inputFields: JSON.stringify(input),
            },
            encryptedFields: [],
        }, key, ENCRYPT);

        expect(secret.encryptedFields).toContain('accessToken');
        expect(secret.encryptedFields).toContain('inputFields');
        expect(secret.encryptedFields).toHaveLength(2);

        expect(secret.value.accessToken).not.toEqual(accessToken);
        expect(JSON.parse(secret.value.inputFields)).not.toEqual(input);
        ...
        expect(secret.value.accessToken).toEqual(accessToken);
        expect(JSON.parse(secret.value.inputFields)).toEqual(input);
