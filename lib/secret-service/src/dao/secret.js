        ...
        if (secret.value[field]) {
            if (method === ENCRYPT) {
                if (!conf.crypto.isDisabled) {
                    ...
                    secret.value[field] = crypto[ENCRYPT](secret.value[field], key);
                }
            } else if (secret.encryptedFields.indexOf(field) !== -1) {
                pull(secret.encryptedFields, field);
                secret.value[field] = crypto[DECRYPT](secret.value[field], key);
            }
        }
    });
    return secret;
...
const fetch = (secret, key, iter = 0) => new Promise(async (resolve, reject) => {
    try {
        if (!secret.lockedAt) {
            let _secret = await Secret[secret.type].findOneAndUpdate(
                { _id: secret._id, lockedAt: { $eq: null } },
...
module.exports = {
    ...
    async getRefreshed(secret, key) {
        ...
        } else if (secret.type === SESSION_AUTH) { // TODO: Needs to be separate?
            _secret = await fetch(secret, key);
        }
        ...
        _secret = _secret.encryptedFields && _secret.encryptedFields.length > 0
            ? cryptoSecret(_secret, key, DECRYPT, _secret.encryptedFields
                .filter((e) => e !== 'refreshToken')) : _secret;
