...
class resolverCopyNew extends Resolver {
  applyResolution(valueA, valueB) {
    if(valueA === null) return { value: valueB };
    return { value: valueA };
  }
}
...
class resolverIfUpdateObject extends Resolver {
  applyResolution(valueA, valueB) {
    if (typeof valueA !== 'object') return { value: valueB };
    if (typeof valueB !== 'object') return false;
    const keys = Object.keys(valueA);
    if (keys.length === 0) return { value: valueB };
    let empty = 0;
    for (key in keys) {
      if (!(key in valueB)) return false;
      if (!valueA[key]) empty += 1;
    }
    if (empty < keys.length) return { value: valueA };
    return { value: valueB };
  }
}
...
class resolverOnlyOverwriteEmpty extends Resolver {
  applyResolution(valueA, valueB) {
    if (valueB === null || valueB === '') return { value: valueA };
    return { value: valueB };
  }
}
