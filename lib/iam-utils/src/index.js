...
module.exports = {
    getUserData: async ({ token, introspectType }) => {
        ...
        try {
            ...
            user.isAdmin = user.permissions.includes(PERMISSIONS.restricted.all);
            user.isTenantAdmin = user.permissions.includes(PERMISSIONS.common["tenant.all"]);
    ...
    hasAll: ({ user, requiredPermissions }) => {
         ...
         */
        if (isAdmin(user) || (isTenantAdmin(user) && areAllCommonPermissions(requiredPermissions)) || allRequiredElemsExistsInArray(permissions, requiredPermissions)) {
            return true;
        }
    ...
    isOwnerOf({ entity, user }) {
        ...
        const tenantIsOwner = (entity.tenant.toString() === user.tenant.toString()) || !!entity.owners.find(
