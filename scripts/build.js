...
services.forEach((service) => {
    ...
    try {
        ...
        if (service.buildScript) execSync(`cd ${service.path} && yarn build`, { stdio: [0, 1, 2] })
        execSync('echo $DOCKER_PASSWORD | docker login -u $DOCKER_USERNAME --password-stdin', { stdio: [0, 1, 2] });
        execSync(`cd ${service.path} && yarn run build:docker`, { stdio: [0, 1, 2], env: Object.assign({VERSION: service.version}, process.env) });
