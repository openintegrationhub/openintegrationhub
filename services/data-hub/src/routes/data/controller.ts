...
export default class DataController {
  ...
  public async getMany(ctx: RouterContext): Promise<void> {
    ...
    const [data, total] = await Promise.all([
      await DataObject.find(condition)
        .skip(paging.offset)
      ...
      for (
        ...
        for (let i = 0; i < body.functions.length; i++) {
          if (body.functions[i].name && body.functions[i].name in handlers) {
            preparedDoc = await handlers[body.functions[i].name](
              preparedDoc,
              body.functions[i].fields,
              condition
            )
          } else {
        ...
        if (preparedDoc) {
          // Update db;
          const result = await DataObject.findOneAndUpdate(
            { _id: doc._id },
            preparedDoc,
            { new: true }
          )
  ...
  public async putOne(ctx: RouterContext): Promise<void> {
    ...
    // @ts-ignore: TS2345
    Object.keys(dataObject.toJSON()).forEach(
      (key: keyof IDataObjectDocument) => {
        if (key === "_id") {
          return
        }
  ...
  public async getStatistics(ctx: RouterContext): Promise<void> {
    ...
    const enrichmentCursor = await DataObject.find(enrichmentCondition)
      .lean()
      .cursor()

    for (
      let doc = await enrichmentCursor.next();
      doc !== null;
      doc = await enrichmentCursor.next()
    ) {
      ...
      if (doc.enrichmentResults.knownDuplicates)
        allDuplicates = allDuplicates.concat(
          doc.enrichmentResults.knownDuplicates
        )
      if (doc.enrichmentResults.knownSubsets)
        allSubsets = allSubsets.concat(doc.enrichmentResults.knownSubsets)
    }
