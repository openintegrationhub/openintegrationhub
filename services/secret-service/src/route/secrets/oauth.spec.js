...
describe('oauth', () => {
    beforeAll(async () => {
        ...
        server = new Server({
            mongoDbConnection: global.__MONGO_URI__.replace('changeme', 'oauth'),
            port,
        });
        await server.start();
        iamMock.setup();
        ...
        const authClient = (await request.post('/auth-clients')
            ...
            .send({
                data: {
                    ...
                    signatureMethod: 'foo',
                    endpoints: {
                        request: 'https://example.com/getRequestToken',
                        access: 'https://example.com/getAccessToken',
                        authorize: 'https://example.com/authorizeToken',
                    },
                    redirectUri: `http://localhost:${conf.port}/callback`,
                    secret: 'foo',
                    key: 'bar',
                    type: 'OA1_THREE_LEGGED',
