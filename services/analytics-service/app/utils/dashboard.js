...
async function getRefs(id, recordUid, token) {
  ...
  const url = id ? `${config.dataHubUrl}/data/${id}` : `${config.dataHubUrl}/data/recordId/${recordUid}`;
    ...
    if (!flows[i].graph || !flows[i].graph.nodes) {
      ...
      for (let j = 0; j < flows[i].graph.nodes.length; j += 1) {
        if (!flows[i].graph.nodes[j].nodeSettings) {
          ...
          break;
        } else if ('analytics' in flows[i].graph.nodes[j].nodeSettings) {
          if (flows[i].graph.nodes[j].nodeSettings.analytics !== true) {
            affectedFlows.push({
              flowId: flows[i].id,
              reason: 'Analytics is not set to true',
              flowData: flows[i],
            });
            break;
          }
...
function drawGraph(graph) {
  ...
  <head>
    ...
    <link rel="stylesheet" href="${config.analyticsServiceBaseUrl}/static/graph.css">
    <script type="text/javascript" src="${config.analyticsServiceBaseUrl}/static/graph.js"></script>
    <style>
    </style>
    <script src="${config.analyticsServiceBaseUrl}/static/cytoscape.min.js"></script>
  </head>
  <body>
