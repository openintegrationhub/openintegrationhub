...
class Server {
  ...
  async setupCors() {
    ...
    // enables CORS
    this.app.use('/event', cors(corsOptions));
  }

  async setupMiddleware() {
    log.info('Setting up middleware');

    this.app.use('/event', iamMiddleware.middleware);
    this.app.use('/storedFunction', iamMiddleware.middleware);
    this.app.use('/applyPolicy', iamMiddleware.middleware);
    this.app.use('/dashboard', iamMiddleware.middleware);

    // This middleware compiles the relevant membership ids generated by the IAM iam middleware and passes them on
    this.app.use('/event', async (req, res, next) => {
