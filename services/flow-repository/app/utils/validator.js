...
function validate(flow) {
  ...
  if (flow.graph && flow.graph.edges && flow.graph.edges.length > 0) {
    for (let i = 0; i < flow.graph.edges; i += 1) {
      if (!findNode(flow.graph.edges[i].source, flow)) {
        errors.push({ message: `Flow source with id ${flow.graph.edges[i].source} could not be found among nodes`, code: 400 });
      }

      if (!findNode(flow.graph.edges[i].target, flow)) {
        errors.push({ message: `Flow target with id ${flow.graph.edges[i].target} could not be found among nodes`, code: 400 });
      }
    }
  }
